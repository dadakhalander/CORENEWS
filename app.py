...
#Database Connection
def dbConnection():
    connection = pymysql.connect(host="localhost", user="root", password="root", database="news")
    return connection
...
def city():
    ...
    if 'user' in session:
        ...
        print("location is ",location)
        if location is None:
            return render_template('city.html', user=session['user'], loc=loc,userlocation=locationis)
        Corpus = pd.read_csv(r"allnews3.csv",encoding='latin-1',error_bad_lines=False)
        print("newspaperis "+newspaper)
        Corpus=Corpus[(Corpus['label']==location+"") & (Corpus['np']==newspaper)]
